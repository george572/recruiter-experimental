...
import { categoryPath, getCategoryBySlug } from "@/lib/taxonomy"
import {
  SITE_NAME,
  absoluteUrl,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${cat.name} ვაკანსიები`
  const description = `აქტუალური ${cat.name} ვაკანსიები საქართველოში. იპოვე სამუშაო კატეგორიაში „${cat.name}“ და გააგზავნე CV პირდაპირ დამსაქმებელთან ${SITE_NAME}-ზე.`
  const canonical = categoryPath(cat.slug)
...
export default async function CategoryPage({ params }: Props) {
  ...
  const data = await loadListingData({ categoryName: cat.name })
  const count = data.total || data.jobs.length
  const heading = `${cat.name} ვაკანსიები საქართველოში`
  const initialFilters = { ...DEFAULT_SAMUSHAO_FILTERS, categories: [cat.name] }
  ...
  return (
    <>
      ...
      <div className="sr-only">
        <h1>{heading}</h1>
        <p>
          {`${SITE_NAME} — ${cat.name} მიმართულების აქტუალური ვაკანსიები საქართველოში.`}
          {count > 0
            ? ` ხელმისაწვდომია ${count}+ პოზიცია სხვადასხვა ქალაქში.`
            : ""}
